...
# Optional: Qwen2-VL has a different decoder layer type
try:
    from transformers.models.qwen2_vl.modeling_qwen2_vl import Qwen2VLDecoderLayer
    _DECODER_LAYER_TYPES = (LlamaDecoderLayer, Qwen2DecoderLayer, MistralDecoderLayer, Qwen2VLDecoderLayer)
except ImportError:
    _DECODER_LAYER_TYPES = (LlamaDecoderLayer, Qwen2DecoderLayer, MistralDecoderLayer)
...
from src.quantization.mxint import MXChannelWiseWeightQuantizer, MXINTActivationQuantizer
from src.quantization.inverted_mxint import InvertedMXChannelWiseWeightQuantizer, InvertedMXINTActivationQuantizer
...
WEIGHT_QUANTIZER_MAP = {
    ...
    "inverted_mxint_quant": InvertedMXChannelWiseWeightQuantizer,
...
INPUT_QUANTIZER_MAP = {
    ...
    "mxint_quant": MXINTActivationQuantizer,
    "inverted_mxint_quant": InvertedMXINTActivationQuantizer,
...
class MinMaxHook:
    ...
    def __call__(self, module, input_batch, output_batch):
        ...
        # Compute min/max and move to CPU immediately to reduce GPU memory pressure
        curr_max = torch.max(tensor, dim=0)[0].float().cpu()
        curr_min = torch.min(tensor, dim=0)[0].float().cpu()
        
        # Explicitly delete intermediate tensor to free GPU memory
        del tensor
...
class LMPTQ(Execute):
    ...
    def get_weight_quantizer(self, **kwargs):
        if self.wqtype == "inverted_mxint_quant":
            # Use w_block_size/w_ebit if specified, otherwise fall back to block_size/ebit
            block_size = self.config["quantization"].get("w_block_size",
                         self.config["quantization"].get("block_size", 32))
            ebit = self.config["quantization"].get("w_ebit",
                   self.config["quantization"].get("ebit", None))
            mantissa_group_size = self.config["quantization"].get("w_mantissa_group_size",
                                  self.config["quantization"].get("mantissa_group_size", 8))
            sub_ebit = self.config["quantization"].get("w_sub_ebit",
                       self.config["quantization"].get("sub_ebit", None))
            if "num_channels" in kwargs:
                del kwargs["num_channels"]
            return WEIGHT_QUANTIZER_MAP[self.wqtype](nbit=self.wbit, train_flag=True, unsigned=False, block_size=block_size, ebit=ebit, mantissa_group_size=mantissa_group_size, sub_ebit=sub_ebit, **kwargs)
        elif self.wqtype == "mxint_quant":
            block_size = self.config["quantization"].get("w_block_size",
                         self.config["quantization"].get("block_size", 32))
            shared_exp_bits = self.config["quantization"].get("shared_exp_bits", 5)
            shared_exp_relative = self.config["quantization"].get("shared_exp_relative", False)
            if "num_channels" in kwargs:
                del kwargs["num_channels"]
            return WEIGHT_QUANTIZER_MAP[self.wqtype](nbit=self.wbit, train_flag=True, unsigned=False, block_size=block_size, shared_exp_bits=shared_exp_bits, shared_exp_relative=shared_exp_relative, **kwargs)
        elif self.wqtype in ("mxfp4_quant", "mxfp4_ceil_quant", "mxfp4_plus_quant", "mxfp4_plus_ceil_quant", "amxfp4_quant"):
            block_size = self.config["quantization"].get("w_block_size",
                         self.config["quantization"].get("block_size", 32))
            if "num_channels" in kwargs:
                del kwargs["num_channels"]
            return WEIGHT_QUANTIZER_MAP[self.wqtype](nbit=4, train_flag=True, unsigned=False, block_size=block_size, **kwargs)
        elif self.wqtype == "nvfp4_quant":
            block_size = self.config["quantization"].get("w_block_size",
                         self.config["quantization"].get("block_size", 16))
            if "num_channels" in kwargs:
                del kwargs["num_channels"]
            return WEIGHT_QUANTIZER_MAP[self.wqtype](nbit=4, train_flag=True, unsigned=False, block_size=block_size, **kwargs)
        else:
            return WEIGHT_QUANTIZER_MAP[self.wqtype](nbit=self.wbit, train_flag=True, unsigned=False, **kwargs)

    def get_input_quantizer(self, **kwargs):
        if self.xqtype == "inverted_mxint_quant":
            # Use a_block_size/a_ebit if specified, otherwise fall back to block_size/ebit
            block_size = self.config["quantization"].get("a_block_size",
                         self.config["quantization"].get("block_size", 32))
            ebit = self.config["quantization"].get("a_ebit",
                   self.config["quantization"].get("ebit", None))
            mantissa_group_size = self.config["quantization"].get("a_mantissa_group_size",
                                  self.config["quantization"].get("mantissa_group_size", 8))
            sub_ebit = self.config["quantization"].get("a_sub_ebit",
                       self.config["quantization"].get("sub_ebit", None))
            return INPUT_QUANTIZER_MAP[self.xqtype](nbit=self.abit, train_flag=True, unsigned=False, block_size=block_size, ebit=ebit, mantissa_group_size=mantissa_group_size, sub_ebit=sub_ebit, **kwargs)
        elif self.xqtype == "mxint_quant":
            block_size = self.config["quantization"].get("a_block_size",
                         self.config["quantization"].get("block_size", 32))
            shared_exp_bits = self.config["quantization"].get("shared_exp_bits", 5)
            shared_exp_relative = self.config["quantization"].get("shared_exp_relative", False)
            return INPUT_QUANTIZER_MAP[self.xqtype](nbit=self.abit, train_flag=True, unsigned=False, block_size=block_size, shared_exp_bits=shared_exp_bits, shared_exp_relative=shared_exp_relative, **kwargs)
        elif self.xqtype in ("mxfp4_quant", "mxfp4_ceil_quant", "mxfp4_plus_quant", "mxfp4_plus_ceil_quant", "amxfp4_quant"):
            block_size = self.config["quantization"].get("a_block_size",
                         self.config["quantization"].get("block_size", 32))
            return INPUT_QUANTIZER_MAP[self.xqtype](nbit=4, train_flag=True, unsigned=False, block_size=block_size, **kwargs)
        elif self.xqtype == "nvfp4_quant":
            block_size = self.config["quantization"].get("a_block_size",
                         self.config["quantization"].get("block_size", 16))
            return INPUT_QUANTIZER_MAP[self.xqtype](nbit=4, train_flag=True, unsigned=False, block_size=block_size, **kwargs)
        else:
            return INPUT_QUANTIZER_MAP[self.xqtype](nbit=self.abit, train_flag=True, unsigned=False, **kwargs)
    ...
    def inject_quantizers(self):
        ...
        for n, m in self.model.named_modules():
            if isinstance(m, _QBaseLinear):
                ...
                weight_quantizer = self.get_weight_quantizer(num_channels=1).to(torch.float16)
                input_quantizer = self.get_input_quantizer().to(torch.float16)
    ...
    def forward_model(self):
        for batch in tqdm(self.calib_set):
            ...
            logits = self.forward_step(batch.long())

            shift_logits = logits[:, :-1, :].contiguous().float()
            self.metric.update(shift_logits, shift_labels)

        ppl = self.metric.reduce()
        self.logger.info(f"[Sanity Check] Perplexity on the Calib Set = {ppl.item():.3f}")
...
class SmoothQuant(LMPTQ):
    ...
    def smooth_fcs_llama_like(self, fcs, act_scales, alpha=0.5):
        ...
        for fc in fcs:
            ...
            fc.wq = self.get_weight_quantizer().to(torch.float16)
            fc.aq = self.get_input_quantizer().to(torch.float16)
    ...
    def llama_attn(self, module:QLlamaAttention, name:str, scales:Dict):
        ...
        module.o_proj.wq = self.get_weight_quantizer().to(self.device)
        module.o_proj.aq = self.get_input_quantizer().to(self.device)
    ...
    def llama_mlp(self, module:QLlamaMLP, name:str, scales:Dict):
        ...
        # special case for the down_proj
        module.down_proj.wq = self.get_weight_quantizer().to(self.device)
        module.down_proj.aq = self.get_input_quantizer().to(self.device)
        return mlp_smooth_scale

    @torch.no_grad
    def inject_quantizers(self, act_scales:Dict):
        for n, m in self.model.named_modules():
            if isinstance(m, _DECODER_LAYER_TYPES):
