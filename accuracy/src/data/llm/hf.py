...
class HFMultipleChoice(DataStage):
    """
    Multiple-choice tasks loaded from the HuggingFace hub.

    Each doc is formatted following the lm-eval-harness loglikelihood
    convention:
        {"options": [(context, continuation), ...], "gold": int}
    """
    hf_path = None
    hf_name = None
    ...
    def load_dataset(self):
        return datasets.load_dataset(self.hf_path, self.hf_name, split=self.data_split)
    ...
    def run(self):
        ...
        docs = [self.format_doc(doc) for doc in dataset]
        print(f"Number of samples in {self.dataset_name} ({self.data_split}) = {len(docs)}")
        return docs
...
class WinoGrande(HFMultipleChoice):
    ...
    def format_doc(self, doc):
        # partial evaluation: the option fills the blank in the context,
        # the shared continuation is the sentence suffix after the blank
        sentence = doc["sentence"]
        pronoun_loc = sentence.index("_")
        target = " " + sentence[pronoun_loc + 1:].strip()
        options = [
            (sentence[:pronoun_loc] + doc["option1"], target),
            (sentence[:pronoun_loc] + doc["option2"], target),
        ]
        return {"options": options, "gold": {"1": 0, "2": 1}[doc["answer"]]}
...
class HellaSwag(HFMultipleChoice):
    ...
    @staticmethod
    def preprocess(text):
        # lm-eval-harness hellaswag preprocessing
        text = text.strip()
    ...
    def format_doc(self, doc):
        ...
        context = self.preprocess(doc["activity_label"] + ": " + ctx)
        options = [(context, " " + self.preprocess(ending)) for ending in doc["endings"]]
        return {"options": options, "gold": int(doc["label"])}

class ARCc(HFMultipleChoice):
    ...
    def format_doc(self, doc):
        ...
        options = [(context, " " + choice) for choice in doc["choices"]["text"]]
        answer_key = self.NUM_TO_LETTER.get(doc["answerKey"], doc["answerKey"])
        labels = [self.NUM_TO_LETTER.get(l, l) for l in doc["choices"]["label"]]
        return {"options": options, "gold": labels.index(answer_key)}
...
class C4(DataStage):
    """
    C4 validation subset for perplexity evaluation (GPTQ convention):
    only the first validation shard is loaded; the evaluator samples
    random 2048-token segments from it with a fixed seed.
    """
    def __init__(self, config_dir):
        super().__init__(config_dir)
    ...
    def run(self):
        dataset = self.load_dataset()
        print(f"Number of samples in the C4 validation shard = {len(dataset)}")
        return dataset
