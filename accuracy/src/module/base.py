...
class _QBaseConv2d(nn.Conv2d):
    ...
    def freeze_weight(self):
        """Pre-apply fake quantization to weights in-place, then replace wq with identity.

        After calling this, trainFunc() still calls self.wq(self.weight) but the quantizer
        is now a no-op (identity), avoiding redundant re-quantization during evaluation.
        Original weights are not recoverable — use only for evaluation.
        """
        with torch.no_grad():
            # Move weight and quantizer to CPU to avoid GPU OOM during fake-quant
            orig_device = self.weight.device
            w_cpu = self.weight.data.cpu()
            wq_cpu = self.wq.cpu().float()
            self.weight.data = wq_cpu(w_cpu).detach().to(orig_device)
            del w_cpu, wq_cpu
        self.wq = _QBase(nbit=self.wbit)
    ...
    def trainFunc(self, x:torch.Tensor):
        ...
        # Handle mixed precision (autocast)
        if torch.is_autocast_enabled():
            target_dtype = torch.get_autocast_gpu_dtype()
            if xq.dtype != target_dtype:
                xq = xq.to(target_dtype)
            if wq.dtype != target_dtype:
                wq = wq.to(target_dtype)
            if bias is not None and bias.dtype != target_dtype:
                bias = bias.to(target_dtype)
            if mask is not None and mask.dtype != target_dtype:
                mask = mask.to(target_dtype)
        
        # Fallback for non-autocast mixed precision
        elif xq.dtype != wq.dtype:
            wq = wq.to(xq.dtype)
            if bias is not None: bias = bias.to(xq.dtype)
            if mask is not None: mask = mask.to(xq.dtype)

        y = F.conv2d(xq, wq.mul(mask), bias, self.stride, self.padding, self.dilation, self.groups)
        return y
...
class _QBaseLinear(nn.Linear):
    ...
    def __init__(self, 
        ...
        # masks: lazily materialized by the pruner (an eager ones_like(weight)
        # buffer doubles weight memory and is unused in the linear forward)
    ...
    def inference(self):
        ...
        # Use FloatActWeight if quantizers output dequantized float values
        # (e.g., MXINT quantizers), or if precision is greater than 8 bits
        use_float_ops = (
            self.aq.nbit > 8 or 
            self.wq.nbit > 8 or
            getattr(self.aq, 'dequantize', False) or
            getattr(self.wq, 'dequantize', False)
        )
    ...
    def trainFunc(self, x:torch.Tensor):
        ...
        # Fallback
        elif x.dtype != wq.dtype:
            wq = wq.to(x.dtype)
            if bias is not None: bias = bias.to(x.dtype)

        y = F.linear(x, wq, bias)
        return y
...
def freeze_model_weights(model):
    """Freeze all quantized weights by pre-computing fake quantization.

    Iterates over all _QBaseLinear and _QBaseConv2d modules and calls
    freeze_weight(), which bakes the quantized weight in-place and replaces
    the weight quantizer with an identity. This eliminates redundant
    weight re-quantization during autoregressive generation.
    """
    import gc
    count = 0
    for module in model.modules():
        if isinstance(module, (_QBaseLinear, _QBaseConv2d)):
