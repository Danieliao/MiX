module MXFP4_SystolicArray #(
    ...
    , input logic [ROWS-1:0][shared_exp_bits-1:0] mxfp4_a_exp_in
    , input logic [ROWS-1:0][group_size-1:0][elem_bits-1:0] mxfp4_a_elem_in
    , input logic [COLS-1:0][shared_exp_bits-1:0] mxfp4_b_exp_in
    ...
    , output logic [ROWS-1:0][shared_exp_bits-1:0] mxfp4_a_exp_out
    , output logic [ROWS-1:0][group_size-1:0][elem_bits-1:0] mxfp4_a_elem_out
    , output logic [COLS-1:0][shared_exp_bits-1:0] mxfp4_b_exp_out
    ...
    logic [ROWS-1:0][COLS:0][shared_exp_bits-1:0] mxfp4_a_exp_wire;
    logic [ROWS-1:0][COLS:0][group_size-1:0][elem_bits-1:0] mxfp4_a_elem_wire;

    // Weight flows top-to-bottom across rows
    logic [ROWS:0][COLS-1:0][shared_exp_bits-1:0] mxfp4_b_exp_wire;
    logic [ROWS:0][COLS-1:0][group_size-1:0][elem_bits-1:0] mxfp4_b_elem_wire;
    ...
    always_comb begin
        for (int r = 0; r < ROWS; r++) begin
            mxfp4_a_exp_wire[r][0]  = mxfp4_a_exp_in[r];
            mxfp4_a_elem_wire[r][0] = mxfp4_a_elem_in[r];
        end

        for (int c = 0; c < COLS; c++) begin
            mxfp4_b_exp_wire[0][c]  = mxfp4_b_exp_in[c];
            mxfp4_b_elem_wire[0][c] = mxfp4_b_elem_in[c];
        ...
        for (r = 0; r < ROWS; r++) begin : gen_row
            for (c = 0; c < COLS; c++) begin : gen_col
                MXFP4_PE #(
                    ...
                    , .rst_n(rst_n)
                    , .acc_shift(acc_shift)
                    , .mxfp4_a_exp_in(mxfp4_a_exp_wire[r][c])
                    , .mxfp4_a_elem_in(mxfp4_a_elem_wire[r][c])
                    , .mxfp4_b_exp_in(mxfp4_b_exp_wire[r][c])
                    , .mxfp4_b_elem_in(mxfp4_b_elem_wire[r][c])
                    , .acc_sign_in(acc_sign_wire[r][c])
                    , .acc_exp_in(acc_exp_wire[r][c])
                    , .acc_mant_in(acc_mant_wire[r][c])
                    , .mxfp4_a_exp_out(mxfp4_a_exp_wire[r][c+1])
                    , .mxfp4_a_elem_out(mxfp4_a_elem_wire[r][c+1])
                    , .mxfp4_b_exp_out(mxfp4_b_exp_wire[r+1][c])
                    , .mxfp4_b_elem_out(mxfp4_b_elem_wire[r+1][c])
    ...
    always_comb begin
        for (int r = 0; r < ROWS; r++) begin
            mxfp4_a_exp_out[r]  = mxfp4_a_exp_wire[r][COLS];
            mxfp4_a_elem_out[r] = mxfp4_a_elem_wire[r][COLS];
        end

        for (int c = 0; c < COLS; c++) begin
            mxfp4_b_exp_out[c]  = mxfp4_b_exp_wire[ROWS][c];
            mxfp4_b_elem_out[c] = mxfp4_b_elem_wire[ROWS][c];
