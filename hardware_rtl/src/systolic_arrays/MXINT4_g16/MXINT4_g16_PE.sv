// Block-32-PE MXINT4 (4.5b): two 16-element sub-blocks (each its own E8M0 scale),
// each normalized to FP32, then summed with the running accumulator in ONE fused
// 3-input FP add.
module MXINT4_g16_PE #(
    ...
    , input logic [group_size/sub_group_size-1:0][mxint_exp_bits-1:0] mxint_a_exp_in
    , input logic signed [group_size-1:0][mxint_mant_bits-1:0] mxint_a_mant_in
    , input logic [group_size/sub_group_size-1:0][mxint_exp_bits-1:0] mxint_b_exp_in
    , input logic signed [group_size-1:0][mxint_mant_bits-1:0] mxint_b_mant_in

    , input logic acc_sign_in
    , input logic [fp_exp_bits-1:0] acc_exp_in
    , input logic [fp_mant_bits-1:0] acc_mant_in

    , output logic [group_size/sub_group_size-1:0][mxint_exp_bits-1:0] mxint_a_exp_out
    , output logic signed [group_size-1:0][mxint_mant_bits-1:0] mxint_a_mant_out
    , output logic [group_size/sub_group_size-1:0][mxint_exp_bits-1:0] mxint_b_exp_out
    , output logic signed [group_size-1:0][mxint_mant_bits-1:0] mxint_b_mant_out
    ...
    localparam DOT_OUT_BITS = (2 * mxint_mant_bits) + LOG2_SUB_SIZE; // 12
    localparam MUL_EXP_BITS = mxint_exp_bits + 1;                // 9
    ...
    MXINT4_g16_MUL #(
        ...
        .clk(clk)
        , .rst_n(rst_n)
        , .mxint_a_exp(mxint_a_exp_in)
        , .mxint_a_mant(mxint_a_mant_in)
        , .mxint_b_exp(mxint_b_exp_in)
        , .mxint_b_mant(mxint_b_mant_in)
        , .out_exp(mul_out_exp)
        , .out_sum(mul_out_sum)
    );

    // Stage 2a: normalize each sub-block (integer-sum normalize)
    always_comb begin
    ...
    // Stage 2b: 3-input FP32 accumulate (fused) — norm[0] + norm[1] + local_acc
    always_comb begin
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            mxint_a_exp_out  <= mxint_a_exp_in;
            mxint_a_mant_out <= mxint_a_mant_in;
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            mxint_b_exp_out  <= mxint_b_exp_in;
            mxint_b_mant_out <= mxint_b_mant_in;
        end
    end
