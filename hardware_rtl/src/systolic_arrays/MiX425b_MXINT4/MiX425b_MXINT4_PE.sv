module MiX425b_MXINT4_PE #(
    ...
    // MXINT4 weight (flows right)
    , input logic [mxint_exp_bits-1:0] mxint_exp_in
    ...
    , output logic [mxint_exp_bits-1:0] mxint_exp_out
    ...
    // Derive DOT_OUT_BITS to match MUL output width (fractional bits = 3, hardcoded to match MUL)
    ...
    localparam DOT_OUT_BITS = ACC_BITS + INVMX_FULL_MANT_BITS;  // = MULT_OUT_BITS = 20
    ...
    // ----------------------------------------------------------------
    // Stage 1: MiX MUL (shift-and-add + 1 multiply, registered)
    // ----------------------------------------------------------------
    MiX425b_MXINT4_MUL #(
    ...
    // ----------------------------------------------------------------
    // Systolic pass-through: MXINT weight flows right
    // ----------------------------------------------------------------
