module MiX45b_MXFP4_PE #(
    ...
    // MXFP4 weight (flows right): 2 groups
    , input logic [group_size/wgt_group_size-1:0][shared_exp_bits-1:0] mxfp4_exp_in
    ...
    , output logic [group_size/wgt_group_size-1:0][shared_exp_bits-1:0] mxfp4_exp_out
    ...
    // ================================================================
    // Stage 2: Parallel normalize (2 weight groups → 2 FP32 values, registered)
    // ================================================================
    ...
    logic [1:0] norm_sign;
    logic [1:0][fp_exp_bits-1:0] norm_exp;
    logic [1:0][fp_mant_bits-1:0] norm_mant;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            norm_sign <= norm_sign_pre;
            norm_exp  <= norm_exp_pre;
            norm_mant <= norm_mant_pre;
        end
    end

    // ================================================================
    // Stage 3: 3-input FP32 accumulate (registered)
    // ================================================================
    ...
    always_comb begin
        if (norm_exp[0] >= norm_exp[1]) begin
            max3_exp = (norm_exp[0] >= local_acc_exp) ? norm_exp[0] : local_acc_exp;
        end else begin
            max3_exp = (norm_exp[1] >= local_acc_exp) ? norm_exp[1] : local_acc_exp;
        end

        diff_a = max3_exp - norm_exp[0];
        diff_b = max3_exp - norm_exp[1];
        diff_c = max3_exp - local_acc_exp;

        mant_a_ext = ((norm_exp[0] == '0) && (norm_mant[0] == '0)) ? '0 : {1'b1, norm_mant[0]};
        mant_b_ext = ((norm_exp[1] == '0) && (norm_mant[1] == '0)) ? '0 : {1'b1, norm_mant[1]};
        ...
        mant_a_signed = norm_sign[0]    ? -$signed({1'b0, mant_a_aligned}) : $signed({1'b0, mant_a_aligned});
        mant_b_signed = norm_sign[1]    ? -$signed({1'b0, mant_b_aligned}) : $signed({1'b0, mant_b_aligned});
