// Self-checking testbench for INT4_PE (uniform INT4, group_size = 32).
//
...
module int4_tb;
    ...
    INT4_PE #(.group_size(group_size), .data_bits(data_bits), .acc_bits(acc_bits)) dut (
    ...
    // random signed 4-bit operand in [-7, 7]
    function automatic logic signed [data_bits-1:0] rand_op();
        return data_bits'(($urandom() % 15) - 7);
    endfunction
