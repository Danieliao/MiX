module MiX45b_MXINT4_MUL #(
    ...
    , parameter mxint_mant_bits = 4     // MXINT4: sign + 3 mantissa
    ...
    , parameter SIGNED_MANT_BITS = mxint_mant_bits  // 4 (input clamped to [-7,7])
    , parameter SHIFTED_BITS = SIGNED_MANT_BITS + 3  // 7
    , parameter LOG2_SUB_GROUP_SIZE = $clog2(sub_group_size)  // 3
    , parameter SUB_ACC_BITS = SHIFTED_BITS + LOG2_SUB_GROUP_SIZE  // 10
    , parameter INVMX_FULL_MANT_BITS = invmx_mant_bits + 1  // 4
    , parameter SUB_MULT_BITS = SUB_ACC_BITS + INVMX_FULL_MANT_BITS  // 14
    , parameter LOG2_SUBS_PER_WGT = $clog2(SUBS_PER_WGT)  // 1
    , parameter MULT_OUT_BITS = SUB_MULT_BITS + LOG2_SUBS_PER_WGT  // 15
