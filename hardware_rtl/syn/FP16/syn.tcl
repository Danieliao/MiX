#==============================================================================
# Synthesis script -- FP16   (top-level module: FP16_SystolicArray)
#
# Reproduces the "FP16" row of ../../results_28nm_iso512.csv.
#
# !! IMPORTANT -- FP16 IS THE ONE DESIGN WHERE THE SYNTHESIZED ARRAY IS NOT 512
#    MAC/cycle. FP16 is a dense 1-MAC-per-PE baseline, and this script builds
#    the FULL 32x32 mesh = 1024 MAC/cycle (the RTL defaults are ROWS=COLS=32).
#    The CSV row is normalized to 512 MAC/cycle (a 16x32 mesh), so:
#
#        CSV Area_um2 = (Total cell area reported here) / 2
#        CSV Power_mW = (Total Power reported here)     / 2
#
#    This script therefore reports about 2x the CSV values: ~1468471 um^2 and
#    ~301.07 mW, which halve to the CSV's 734235.82 um^2 and 150.537 mW.
#
#    Every other design in this artifact is literally 512 MAC/cycle
#    (4x4 PEs x 32 elements) and compares against the CSV directly, with no
#    scaling of any kind.
#
# HOW TO RUN
...
#   3. Read the results:
#        Area  -> log/FP16_SystolicArray.2.28nm.syn_area.28nm.rpt    ("Total cell area", um^2)
#        Power -> log/FP16_SystolicArray.2.28nm.syn_power.28nm.rpt   ("Total Power", mW)
#        Timing-> log/FP16_SystolicArray.2.28nm.syn_timing.28nm.rpt  (slack must be >= 0)
#
...
# Set top level name
set top_level "FP16_SystolicArray"

# RTL source -- relative to this directory, no editing needed
set VERILOG_DIR "../../src/systolic_arrays/FP16"

set RTL_SRC_FILES [list \
"$VERILOG_DIR/FP16_MUL.sv" \
"$VERILOG_DIR/FP16_PE.sv" \
"$VERILOG_DIR/FP16_SystolicArray.sv" \
]
